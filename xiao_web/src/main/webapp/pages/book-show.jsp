					...
					<div class="am-u-sm-12">
						...
						<form class="am-form">
									...
									<td>
										<div class="tpl-form-file-img">
											<c:if test="${book.book_img != null}">
												<img style="height: auto" src="/pic/book/${book.book_img}"/>
												<br/>
											</c:if>
										</div>
									</td>
							...
							<security:authorize access="hasRole('ADMIN')">
								<c:choose>
									<c:when test="${book.borrows.size() != 0}">
										<table id="dataList"
											   class="am-table am-table-striped am-table-hover table-main">
											<thead>
											<tr>
												<th class="table-id">用户名字</th>
												<th class="table-id">联系方式</th>
												<th class="table-id">邮箱</th>
												<th class="table-id">借书时间</th>
												<th class="table-id">应归还时间</th>
												<th class="table-id">是否借出</th>
												<th class="table-id">归还时间</th>
											</tr>
											</thead>
											...
											<c:forEach items="${book.borrows}" var="borrow" >
												<tr>
														...
														<%--                                <td>${record.shops[]}</td>--%>
													<c:forEach items="${borrow.users}" var="user" begin="0" step="1">
														<td>${user.username}</td>
														<td>${user.phoneNum}</td>
														<td>${user.email}</td>
